...
void scheduler(void* arg) {
    ...
    // Create 3 workers
    worker_data_t data1 = {pool, 1, 3};
    worker_data_t data2 = {pool, 2, 4};
    worker_data_t data3 = {pool, 3, 2};
    ...
    for (int round = 0; round < 4; round++) {
        ...
        if (state != 3) sp_co_go(pool, w1);  // Not DEAD
        
        sp_co_state(w2, &state);
        if (state != 3) sp_co_go(pool, w2);
        
        sp_co_state(w3, &state);
        if (state != 3) sp_co_go(pool, w3);
    }
